...
from tests.data.alert_data import ALERTS_SERVICE_A, ALERTS_SERVICE_X, SERVICE_ID_A, SERVICE_ID_X
...
class TestAlerts:
    ...
    def test_get_alerts_all(self):
        ...
        start_ts = 1
        end_ts = 6000
        response = client.get(f"/alerts?service_id={SERVICE_ID_X}&start_ts={start_ts}&end_ts={end_ts}")
    ...
    def test_get_alerts_for_specific_service(self):
        ...
        response = client.get(f"/alerts?service_id={SERVICE_ID_A}&start_ts={start_ts}&end_ts={end_ts}")
        assert response.status_code == 200
        assert len(response.json()["alerts"]) == 1
